 ...
 *   dos_kbhit() -> AX flag : input poll (kbhit-style; nonzero when a key is up)
 *   dos_getch() -> AX      : blocking key read (getch-style), returns scancode/char
 *   dos_sound(int ms)      : delay(ms) busy-wait
 *   dos_nosound(void)        : delay/sound teardown paired with dos_sound
...
int menu_draw(void)
{
    ...
    draw_sprite_xor(TEXT_X0, ROW_Y0, font_img);
    ...
    for (;;) {
        ...
        key = (unsigned char)(idle_wait_key() & 0xff);
        if (key == 0xd) {
            ...
            draw_sprite_xor(TEXT_X0, (row << 3) + ROW_Y0, font_img);
            continue;
        }

        /* Non-Enter: if another key is queued, read it (getch-style). */
        if (dos_kbhit() != 0)
            key = (unsigned char)(dos_getch() & 0xff);
